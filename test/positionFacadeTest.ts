...
describe("## Verify the Positions Facade ##", () => {
  ...
  describe("Verify the addOrUpdatePosition method", () => {
    it("It should not find update xxx@b.dk's position document", async () => {
      await expect(positionFacade.addOrUpdatePosition("xxx@b.dk", 2, 3)).to.be.rejectedWith(ApiError)
    })
  })

  // describe("Verify the findNearbyPositions method", () => {
  //   it("Should find ", async () => {
  //     const result = await positionFacade.findNearbyPositions("pp@b.dk", 12.48, getLatitudeInside(55.77, DIST_TO_SEARCH), DIST_TO_SEARCH)
  ...
  describe("Verify the findNearbyFriends method", () => {
    it("Should Not find ", async () => {
      ...
      expect(result.length).to.be.equal(1)
      expect(result[0].name).to.be.equal("Donald Duck")
      expect(result[0].location.coordinates[0]).to.be.equal(12.48)
    })
  })
